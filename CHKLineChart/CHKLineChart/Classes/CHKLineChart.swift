...
@objc public protocol CHKLineChartDelegate: class {
    ...
    ///   - index: 点击的位置
    ///   - item: 数据对象
    @objc optional func kLineChartDidUnSelect(chart: CHKLineChartView)
    ...
    @objc optional func kLineChart(chart: CHKLineChartView, didScrollToIndex index: Int)
...
open class CHKLineChartView: UIView {
    ...
    /// MARK: - 常量
    let kMinRange = 20       //最小缩放范围
    let kMaxRange = 120     //最大缩放范围
    ...
    open var enableLong: Bool = true {
        didSet {
            self.showSelection = self.enableLong
        }
    }
    ...
    open var showSelection: Bool = true {
        didSet {
            ...
            if !showSelection {
                self.delegate?.kLineChartDidUnSelect?(chart: self)
            }
        }
    }
    ...
    var rangeFrom: Int = 0 {        //可见区域的开始索引位
        didSet {
            self.delegate?.kLineChart?(chart: self, didScrollToIndex: rangeFrom)
        }
    }
    ...
    open var style: CHKLineChartStyle! {           //显示样式
        didSet {
            ...
            self.enableLong = self.style.enableLong
    ...
    public init(frame: CGRect, delegate: CHKLineChartDelegate) {
        super.init(frame: frame)
...
//    }
    ...
    func setSelectedIndexByPoint(_ point: CGPoint) {
        ...
        guard self.enableTap || self.enableLong else {
            return
        }
